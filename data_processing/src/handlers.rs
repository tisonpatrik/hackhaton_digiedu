...
// Helper function to transcribe audio (extracted from transcribe_audio handler)
async fn transcribe_audio_file(audio_path: &Path) -> Result<String, String> {
    // Create directories
...
mod tests {
    ...
    async fn test_upload_file_exists() {
        ...
        assert_eq!(body.file_type, "other");
        assert!(body.transcript_path.is_none());
    }
